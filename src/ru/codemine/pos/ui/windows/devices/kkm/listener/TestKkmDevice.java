...
@Component
public class TestKkmDevice implements ActionListener
{
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        try
        {
            Cheque testCheque = new Cheque();
            testCheque.setCreator(new User("Тест", "Тест"));
            kkmService.createKkm(device).printCheque(testCheque);
        } 
        catch (KkmException ex)
        {
            WebOptionPane.showMessageDialog(window, ex.getLocalizedMessage(), "Ошибка печати тестового чека", WebOptionPane.WARNING_MESSAGE);
            return;
        }
        
        WebOptionPane.showMessageDialog(window, "Печать тестового чека завершена", "", WebOptionPane.WARNING_MESSAGE);
    }
